...
class employeeClass:
    def __init__(self,root):
        self.root=root
        self.root.geometry("1100x550+220+100")
        ...
        #===Content===
        #calling chekconact function
        validate_contact=self.root.register(self.checkcontact)
        #===Row1===
        lbl_empid=Label(self.root,text="Emp Id",font=("goudy old style",15),bg="white").place(x=50,y=150)
        lbl_gender=Label(self.root,text="Gender",font=("goudy old style",15),bg="white").place(x=350,y=150)
        lbl_contact=Label(self.root,text="Contact",font=("goudy old style",15),bg="white").place(x=750,y=150)
        txt_empid=Entry(self.root,textvariable=self.var_emp_id,font=("goudy old style",15),bg="lightyellow",validate='key',validatecommand=(validate_contact,"%P")).place(x=150,y=150,width=180)
        ...
        self.txt_contact=Entry(self.root,textvariable=self.var_contact,width=10,font=("goudy old style",15),bg="lightyellow",validate='key',validatecommand=(validate_contact,"%P")).place(x=850,y=150,width=180)
        
        #Contact limitsize function
        def limitSize(*args):
            value =self.var_contact.get()
            if len(value) > 2: self.var_contact.set(value[:10])
        self.var_contact.trace('w', limitSize)
        
        #emp id limitsize function
        def limitempid(*args):
            value =self.var_emp_id.get()
            if len(value) > 2: self.var_emp_id.set(value[:3])
        self.var_emp_id.trace('w', limitempid)
        ...
        txt_salary=Entry(self.root,textvariable=self.var_salary,font=("goudy old style",15),bg="lightyellow",validate="key", validatecommand=(validation, '%P')).place(x=600,y=270,width=180)

        
        #Buttons===
        btn_save=Button(self.root,text="Save",command=self.add,font=("goudy old style",15),bg="#2196f3",fg="white",cursor="hand2").place(x=620,y=305,width=110,height=28)
        btn_udate=Button(self.root,text="Update",command=self.update,font=("goudy old style",15),bg="#4caf50",fg="white",cursor="hand2").place(x=740,y=305,width=110,height=28)
        ...
        btn_clear=Button(self.root,text="Clear",command=self.clear,font=("goudy old style",15),bg="#607d8b",fg="white",cursor="hand2").place(x=980,y=305,width=110,height=28)        
        btn_check=Button(self.root,text="Check",command=self.validation,font=("goudy old style",15),bg="green",fg="white",cursor="hand2").place(x=500,y=305,width=110,height=28)
        
        #===Employee Details table frame=====
...
#===================================All functions=======================================================
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get()=="" :
                messagebox.showerror("Error","Fill all the details of employee",parent=self.root)
    ...
    #Show employee details function in employee table
    def show(self):
        con=sqlite3.connect(database=r'ims.db')
        cur=con.cursor()
        try:
    ...
    #Get employee details function in employee table
    ...
    def update(self):
        ...
        try:
            if self.var_emp_id.get()=="" :
                messagebox.showerror("Error","Please select employee",parent=self.root)     
            else:
                cur.execute("Select * from employee where eid=?",(self.var_emp_id.get(),))
                row=cur.fetchone()
                if row==None:
...
#Validation functions
    ...
    def checkpassword(self,password):
        if len(password)<=8:
            if re.match("^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z](?=.*[^a-bA-B0-9]))",password):
                ...
                messagebox.showerror("Invalid","Password strength is too weak,example(Neha@123)",parent=self.root)
                return False
        else:
            messagebox.showerror("Invalid","Password Length try to exceed",parent=self.root) 
            return False       
    ...
    def checkmail(self,email):
        if len(email)>7:
            if re.match("^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,5})$",email):
                ...
                messagebox.showwarning("Alert","Enter Valid email",parent=self.root)
                return False
        else:
            messagebox.showerror("Invalid","Email length is too small",parent=self.root) 
            return False 
    ...
    def checkdate(self,date):
        if len(date)<=10:
            if re.match('(19|20)\d\d[- /.](0[1-9]|1[012])[- /.](0[1-9]|[12][0-9]|3[01])',date):
                ...
                messagebox.showerror("Error",'Check format of the date,example(YYYY-MM-DD)',parent=self.root) 
                return False 
        else:
    ...
    def validation(self):
        if self.var_name.get()=='':
            ...
            messagebox.showerror("Error","Please enter date of birth",parent=self.root) 
        elif self.var_doj.get()=='':
            messagebox.showerror("Error","Please enter date of joining",parent=self.root)    
        elif self.var_emp_id.get()=='':
            ...
            messagebox.showerror("Error","Please enter salary",parent=self.root)   
        elif self.var_email.get()!=None and self.var_pass.get()!=None and self.var_dob.get()!=None and self.var_doj.get()!=None :
         
            x=self.checkmail(self.var_email.get())
            y=self.checkpassword(self.var_pass.get())
            z=self.checkdate(self.var_dob.get())
            w=self.checkdate(self.var_doj.get())
